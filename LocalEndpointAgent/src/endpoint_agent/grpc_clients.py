...
def _normalize_grpc_endpoint(raw_url: str) -> tuple[str, bool]:
    ...
    if scheme in {"http", "https", "grpc", "grpcs"}:
        host = parsed.hostname or ""
        if not host:
            raise ValueError(f"Invalid gRPC endpoint: {raw_url}")

        if ":" in host and not host.startswith("["):
            # IPv6 host for gRPC target
            host = f"[{host}]"
...
class ActivityServiceDirectClient:
    def __init__(self, url: str, *, agent_auth_token: str | None = None, agent_auth_header: str = "x-agent-token") -> None:
        self.url = url
        self.target, self.secure = _normalize_grpc_endpoint(url)
        self._agent_auth_token = (agent_auth_token or "").strip()
        self._agent_auth_header = (agent_auth_header or "x-agent-token").strip().lower()
    ...
    async def connect(self) -> None:
        ...
        if self.secure:
            ...
            self._channel = grpc.aio.secure_channel(self.target, creds)
        else:
            self._channel = grpc.aio.insecure_channel(self.target)
        self._stub = activity_pb2_grpc.ActivityGrpcServiceStub(self._channel)
        logger.info("ActivityService gRPC connected: %s (secure=%s)", self.target, self.secure)
...
class AgentManagementDirectClient:
    def __init__(
        ...
        *,
        ...
        agent_auth_header: str = "x-agent-token",
    ...
    async def connect(self) -> None:
        ...
        logger.info("AgentManagementService gRPC connected: %s (secure=%s)", self.target, self.secure)
        logger.info(
